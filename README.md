...
在 `账号` 面板的 `账号记录` 中可以管理本机保存过的网易云账号：

- `使用`：切换到该 UID 的本地数据目录和模型/语音配置，不会自动重新导入。
- `检查连接`：重新验证该账号保存的 Cookie 和网易云 API 服务是否可用。
- `导入`：只对当前正在使用的账号导入喜欢歌曲、歌单和听歌记录。
- `清除`：删除非当前账号的本地运行数据。
- `重置`：清空当前账号的本地运行数据，需要明确确认。
